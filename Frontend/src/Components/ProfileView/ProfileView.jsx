        ...
        <>
            <div className='w-screen fixed h-screen z-30'>
                <div className="bg-[#B03052] border-2 border-[#EBE8DB] fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 p-3 w-60 rounded-md shadow-2xl shadow-gray-600">
                    ...
                    <div className='h-20 w-20 bg-pink-600 border-5 border-pink-400 text-[#EBE8DB] m-auto mt-2 rounded-full flex items-center justify-center font-para text-5xl'>
                        {userName.length > 1 ? userName.slice(0, 2).toUpperCase() : userName}
                    </div>

                    <div className='mt-2 font-para text-md text-[#EBE8DB]'>
                        <div className='flex flex-wrap m-1 border-b border-red-300'>
                            <div className='w-[40%] text-pink-300'>Username</div>
                            <div className='w-[60%] break-words'>{userProfile.username}</div>
                        </div>

                        <div className='flex flex-wrap m-1 border-b border-red-300'>
                            <div className='w-[40%] text-pink-300'>Full name</div>
                            <div className='w-[60%] break-words'>{userProfile.fullname}</div>
                        </div>

                        <div className='flex flex-wrap m-1 border-b border-red-300'>
                            <div className='w-[40%] text-pink-300'>E-mail</div>
                            <div className='w-[60%] break-words'>{userProfile.email}</div>
                        </div>
                    </div>

                    <button onClick={closeProfile} className='greenButton btnColor font-para w-25 p-1 ml-[26%] mt-2 mb-1 cursor-pointer rounded-sm border-2 border-[#EBE8DB]'>Close</button>
                </div>
            </div>
        </>
    )
